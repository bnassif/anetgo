...
var getCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) > 0 {
			...
			if vmid := getOpts.GetInt("vm-id"); vmid != 0 {
...
func init() {
	...
	getCmd.PersistentFlags().IntP("vmid", "v", 0, "Filter snapshots by the target VM")
	...
	_ = getOpts.BindPFlag("vmid", getCmd.PersistentFlags().Lookup("vmid"))
