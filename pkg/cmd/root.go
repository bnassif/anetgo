...
func init() {
	...
	RootCmd.PersistentFlags().StringP("key", "k", "", "The API key to use")
	RootCmd.PersistentFlags().StringP("secret", "s", "", "The API secret to use")
	RootCmd.PersistentFlags().StringP("url", "u", "https://cloudapi.atlantic.net", "The API base URL to use")
	RootCmd.PersistentFlags().StringP("api-version", "V", "2010-12-30", "The API version to target")
	RootCmd.PersistentFlags().IntP("timeout", "T", 30, "HTTP client timeout in seconds")
	RootCmd.PersistentFlags().BoolP("raw", "r", false, "Whether to return the raw, unparsed response from the API")
